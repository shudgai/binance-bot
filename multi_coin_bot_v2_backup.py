...
DEFAULT_SYMBOLS = [
    ...
    "SOLUSDT", "SUIUSDT", "AVAXUSDT", "FETUSDT", "LINKUSDT", "NEARUSDT", "INJUSDT", "WLDUSDT",
...
async def check_position_exits(sym):
    ...
    ai_fresh = (time.time() - getattr(s, "ai_updated_at", 0.0)) < AI_UPDATE_INTERVAL * 2
    signal_is_reverse = (ai_fresh and ai_action == "CLOSE")
    ...
    MIN_HOLD_BEFORE_STOP_SEC = 90
    HARD_STOP_PCT = -0.03
    NORMAL_STOP_PCT = -0.015
    ...
    # 2. 保本：任何時候都允許
    elif getattr(s, "is_breakeven_set", False) and profit_pct <= 0.0005:
        exit_triggered = True
        exit_reason = f"保本防護 (獲利回落至 {profit_pct*100:.2f}%)"
        is_stop_loss = True

    # 3. 持倉未滿 90 秒：擋掉後續所有一般停損、反手、動態停損
    elif hold_sec < MIN_HOLD_BEFORE_STOP_SEC:
        return
    ...
    # (1) AI 總裁平倉指令
    if not exit_triggered and signal_is_reverse:
        exit_triggered = True
        exit_reason = f"AI指令平倉 ({getattr(s, 'ai_reason', '')})"
        is_stop_loss = (profit_pct < 0)
        s.ai_action = "HOLD"

    # (3) 放寬版動態硬停損
    if not exit_triggered:
        dynamic_stop_pct = min(max(s.current_atr * 4.0 / avg, 0.005), 0.010)
        if profit_pct <= -dynamic_stop_pct:
            exit_triggered = True
            exit_reason = f"動態硬停損 (虧損達 {profit_pct*100:.2f}%，門檻: {dynamic_stop_pct*100:.2f}%)"
            is_stop_loss = True

    # (4) 急速崩盤防禦 (Velocity Stop)
    ...
    # (5) 極致鎖利與利潤保護
    MIN_HOLD_FOR_PROFIT_LOCK_SEC = 0
    if not exit_triggered and hold_sec >= MIN_HOLD_FOR_PROFIT_LOCK_SEC:
        if s.highest_profit_pct >= 0.015:
            if profit_pct < s.highest_profit_pct * 0.8:
                exit_triggered = True
                exit_reason = f"極致鎖利 (最高 {s.highest_profit_pct*100:.2f}% -> 現 {profit_pct*100:.2f}%)"
        elif s.highest_profit_pct >= 0.010:
            if profit_pct < s.highest_profit_pct * 0.6:
                exit_triggered = True
                exit_reason = f"極致鎖利 (最高 {s.highest_profit_pct*100:.2f}% -> 現 {profit_pct*100:.2f}%)"
        elif s.highest_profit_pct >= 0.008:
            if profit_pct < s.highest_profit_pct * 0.5:
                exit_triggered = True
                exit_reason = f"極致保本 (最高 {s.highest_profit_pct*100:.2f}% -> 現 {profit_pct*100:.2f}%)"
        elif s.highest_profit_pct >= 0.004:
            if profit_pct <= 0.001:
                exit_triggered = True
                exit_reason = f"保本微利防禦 (最高 {s.highest_profit_pct*100:.2f}% -> 現 {profit_pct*100:.2f}%)"

    # (6) 反向訊號補救
    if not exit_triggered and should_recover_from_reversal(sym, is_long):
        exit_triggered = True
        exit_reason = "方向錯誤且出現反轉訊號"
        is_stop_loss = True

    # (7) 震盪區間極速停利
    if not exit_triggered:
        ai_regime = getattr(s, "ai_regime", "CHOP")
        if ai_regime == "CHOP" and profit_pct >= 0.01:
            exit_triggered = True
            exit_reason = f"震盪網格極速停利 (獲利 {profit_pct*100:.2f}%)"
    ...
    semi_auto_exit = STRATEGY_CONF.get("SEMI_AUTO_EXIT", True)

    # 分批止盈 (如果是半自動且已經平倉50%，其餘由下方移動停利判定)
    if not exit_triggered and s.highest_profit_pct >= base_tp_atr and not s.has_partial_closed:
        if not semi_auto_exit:
            cs = 'sell' if is_long else 'buy'
            logger.info(f"🎯 [分批止盈] {sym} 獲利達 {s.highest_profit_pct*100:.2f}%，先平倉 50% 落袋為安")
            await close_position(sym, cs, abs(s.qty) * 0.5, p, avg, reason="分批止盈(50%)", is_profit=True)
            s.has_partial_closed = True
            return
        else:
            exit_triggered = True
            exit_reason = f"分批止盈提示 (獲利達 {profit_pct*100:.2f}% >= {base_tp_atr*100:.2f}%)"

    # 大波段移動停利 (ATR Trailing Stop)
    if not exit_triggered and s.highest_profit_pct >= base_tp_atr:
        trail_stop_dist = s.current_atr * (3.0 if s.has_partial_closed else 1.5)
        if (is_long and p <= s.trailing_highest - trail_stop_dist) or (not is_long and p >= s.trailing_lowest + trail_stop_dist):
            exit_triggered = True
            exit_reason = f"ATR移動停利 (最高獲利達 {s.highest_profit_pct*100:.2f}%，回落觸發)"

    # 趨勢感知鎖利 (Trend-Aware Trailing)
    if not exit_triggered and s.highest_profit_pct >= base_tp_trend and profit_pct < s.highest_profit_pct * 0.8:
        cur_open = float(s.ohlcv[-1][1]) if len(s.ohlcv) >= 1 else p
        if is_long:
            trend_checks = [s.ema20 > 0 and p > s.ema20, s.macd_hist > 0, p > cur_open]
        else:
            trend_checks = [s.ema20 > 0 and p < s.ema20, s.macd_hist < 0, p < cur_open]
        trend_score = sum(trend_checks)
        if trend_score < 2:
            exit_triggered = True
            exit_reason = f"趨勢翻轉鎖利 (最高獲利 {s.highest_profit_pct*100:.2f}%，趨勢翻轉)"

    # 反向爆量提早下車 (Reverse Volume Explosion Exit)
    if not exit_triggered and s.avg_vol_24h_1m > 0 and s.current_vol > s.avg_vol_24h_1m * 3.0:
        c1, c2 = (s.closes[-2], s.closes[-1]) if len(s.closes) >= 2 else (p, p)
        c_change = abs(c2 - c1) / max(c1, 1e-8)
        if c_change > 0.002:
            is_big_red = c2 < c1
            is_big_green = c2 > c1
            if (is_long and is_big_red and p < getattr(s, 'vwap', 0)) or (not is_long and is_big_green and p > getattr(s, 'vwap', 0)):
                exit_triggered = True
                exit_reason = "反向爆量強制下車"
                is_stop_loss = (profit_pct < 0.0)
    ...
    if not exit_triggered:
        ...
        ema20_broken_long = is_long and p < getattr(s, "ema20", 0)
        ema20_broken_short = not is_long and p > getattr(s, "ema20", float('inf'))
        if (is_long and m_death and ema20_broken_long) or (not is_long and m_golden and ema20_broken_short):
            threshold = -0.008
            if profit_pct < threshold:
                exit_triggered = True
                exit_reason = "MACD反向交叉且達虧損門檻"
                is_stop_loss = True
    ...
    if len(s.pnl_history) == 5:
        ...
        if is_decaying and profit_pct * 100 >= 0.8:
            exit_triggered = True
            exit_reason = "動能衰減落袋為安"

    # ATR 完全獲利出場 (Full ATR TP)
    if not exit_triggered:
    ...
    if exit_triggered:
        ...
        update_trade_exit_advice(sym, None)
        s.highest_profit_pct = 0.0
        if getattr(s, 'advice_time', 0) != 0:
            s.advice_time = 0
            
        logger.warning(f"@@COIN_DEBUG@@ ⚡ {sym} 觸發出場條件，執行即時自動平倉！原因: {exit_reason}")
        await close_position(sym, cs, abs(s.qty), p, avg, reason=exit_reason, is_stop_loss=is_stop_loss, is_profit=not is_stop_loss)
...
def risk_guard(sym, side, route="a"):
    ...
    if side == 'buy' and route == 'a':
        ...
        if getattr(s, "htf_trend", None) != "long":
            logger.info(f"⚠️ [多單過濾] {sym} 1H 趨勢非多頭 ({s.htf_trend})，趨勢限制已放寬，放行開倉")
    ...
    if side == 'sell' and route == 'a':
        ...
        if getattr(s, "htf_trend", None) != "short":
            logger.info(f"⚠️ [空單過濾] {sym} 1H 趨勢非空頭 ({s.htf_trend})，趨勢限制已放寬，放行開倉")
...
def compute_signal_strength(sym):
    ...
    # === 快車道動能算分邏輯 ===
    if getattr(s, "vol_mode", "") == "低波動模式 (Low Vol)":
        # 1. 如果是低波動，代表這是沒有資金成交量的假突破，強行扣分
        strength -= 2.0 
        logger.debug(f"⚠️ {sym} 處於低波動模式，快車道分數強制扣減 2 分，防止指標虛胖。")
    ...
    if getattr(s, "htf_trend", None) and getattr(s, "htf_trend", None) != expected_trend:
        ...
        if not is_ai_reversal:
            strength -= 1.0
            logger.debug(f"⚠️ [大週期逆勢] {sym} 1H 趨勢 ({s.htf_trend}) 與開單方向 ({side}) 不符，短線扣分 (-1.0)")
    ...
    try:
        ...
        if res and len(res) >= 2:
            ...
            if side == 'buy' and latest_close < prev_close:
                logger.info(f"🛑 [方向確認失敗] {sym} 等待期間轉跌，取消做多")
                return False
            if side == 'sell' and latest_close > prev_close:
                logger.info(f"🛑 [方向確認失敗] {sym} 等待期間轉漲，取消做空")
                return False
    except:
        pass
    return True
...
async def check_entries():
    ...
    for sym in open_syms:
        ...
        if s.avg_price > 0 and hasattr(s, "close_price"):
            ...
            if profit < -0.01:  # 浮虧超過 1% 才算嚴重虧損
                losing_count += 1
                
    if losing_count >= 3:
        # logger.debug(f"⚠️ [多幣虧損風控] 已有 {losing_count} 個倉位虧損超過 -1.0%，暫停新開倉")
        # return
        pass
    # --------------------
    ...
    # --- 同車過濾風控 (Concurrent Entry Limit) ---
    now = time.time()
    GLOBAL_STATE["recent_entries"] = [t for t in GLOBAL_STATE.get("recent_entries", []) if now - t < 900]
    if len(GLOBAL_STATE["recent_entries"]) >= 2:
        logger.warning(f"⚠️ [同車防護] 過去 15 分鐘內已連續開倉 {len(GLOBAL_STATE['recent_entries'])} 筆，暫停開新單以防板塊集體滅頂")
        return
    # --------------------
    ...
    for sym in ALL_SYMBOLS:
        ...
        # 日誌減肥：只有分數具備潛力時才印出，過濾掉一堆 0~5 分的雜訊
        if strength >= 6.0:
    ...
    for i in range(min(remaining_slots, len(candidates))):
        ...
        if bypass_pullback:
            ema20 = s.ema20
            price_vs_ema = abs(s.close_price - ema20) / ema20 if ema20 > 0 else 0
            if price_vs_ema > 0.003:  # 偏離 EMA20 超過 0.3%，不追
                logger.info(f"⏳ [追高保護] {sym} 價格偏離EMA20 {price_vs_ema*100:.2f}%，取消強勢直通改掛單等回撤")
                bypass_pullback = False
                
        if not check_direction_ok(sym, side, bypass_pullback, route=route):
            continue
            
        if bypass_pullback:
            logger.info(f"⚡ [強勢直通] {sym} 動能強烈，無須等待回撤，準備市價開倉前確認 K 線...")
            confirmed = await wait_for_confirmation(sym, side)
            if not confirmed:
                continue
