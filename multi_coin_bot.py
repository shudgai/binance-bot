...
def mark_exit(sym, is_stop_loss=False, reason=""):
    ...
    if is_stop_loss:
        ...
        if s["stop_count"] >= MAX_STOPS_IN_WINDOW and (now - s["first_stop_time"]) <= BAN_WINDOW:
            ...
            print(f"🚫 [狀態] {sym} 24h內{MAX_STOPS_IN_WINDOW}次停損 → BANNED 24h")
    ...
    try:
        ...
        if PAPER_TRADING:
            ...
            config = {
                ...
                "step_percent": COIN_PROFILE_CONFIG.get(sym, {}).get("step_percent", 0.0002),  # 降低間距讓成交更貼近市價
                "fee_rate": 0.001,
...
def is_entry_volume_confirmed(sym, side):
    ...
    if rr_ratio < 1.99: # 恢復至原 2.0 盈虧比門檻，確保高盈虧比
...
def is_entry_allowed(sym, side, route="a"):
    ...
    # 均線過濾器已還原 - 嚴格進場模式防止短線逆勢
    if s.get("sma200_15m", 0) > 0:
        ma200 = s["sma200_15m"]
        if side == 'buy' and cp <= ma200:
            print(f"@@COIN_DEBUG@@ 🛑 {sym} 觸發 [MA過濾] 價格 {cp:.4f} <= 15m_SMA200 {ma200:.4f} (禁止低於200MA做多)")
            return False
        if side == 'sell' and cp >= ma200:
            print(f"@@COIN_DEBUG@@ 🛑 {sym} 觸發 [MA過濾] 價格 {cp:.4f} >= 15m_SMA200 {ma200:.4f} (禁止高於200MA做空)")
            return False
    ...
    # 判斷波動太小的條件：當前 ATR 小於 24H 平均 ATR 的 40% (原 60%)，或 BB 區間太窄 0.3% (原 0.5%)
...
def compute_signal_strength(sym):
    ...
    # 限制開倉不要太偏離短期趨勢線，避免追價開倉 (實施動態擴張門檻)
    k_factor = s.get("k_factor", 3.0)
    current_atr = s.get("current_atr", 0.0)
    dynamic_limit = (k_factor * current_atr) / close if close > 0 else 0.05
    current_deviation = abs(close - ema20) / ema20 if ema20 > 0 else 0
    
    if current_deviation > dynamic_limit:
        # print(f"🚫 [擴張過濾] {sym} 拒絕進場：過度乖離 ({current_deviation*100:.2f}% > 門檻 {dynamic_limit*100:.2f}%)")
        close_near_ema20_long = False
        close_near_ema20_short = False
    else:
        close_near_ema20_long = ema20 <= 0 or close <= ema20 * (1 + dynamic_limit)
        close_near_ema20_short = ema20 <= 0 or close >= ema20 * (1 - dynamic_limit)
...
async def check_entries():
    ...
    for sym in ALL_SYMBOLS:
        ...
        if s.get("pending_side"):
            ...
            # 換線了，檢查前一根(訊號K線)是否反轉 (還原嚴格版：必須為實體K線且上/下影線不大於實體 1.5 倍)
            if len(s["ohlcv"]) >= 2:
                prev_candle = s["ohlcv"][-2]
                ...
                if is_valid:
                    print(f"✅ [訊號確認] {sym} {s['pending_side']} 訊號已確認 (K線收盤無反轉且影線比例安全)")
                    side = s["pending_side"]
        ...
        # --- 反手冷卻時間 (min_flip_time / Flip Buffer) 過濾 ---
        ...
        if last_trade_side != "" and side != last_trade_side:
            ...
            min_flip = s.get("min_flip_time", 900)  # 嚴格反手防禦：15分鐘 (900秒)
            if route == "Exhaustion_Entry_Extreme":
                min_flip = 180  # 強反轉放寬至 3 分鐘
...
async def check_engine_stale_orders():
    """
    超時撤單背景掃描器 (Engine-Level Stale Order Sweeper)
    ─────────────────────────────────────────────────
    每 30 秒呼叫 ExecutionEngine.cancel_stale_pending_orders()，
    清理在 ExecutionEngine.pending_orders 中捨備超過 60 秒仳未成交的限價進場單。
    
    撤單後根據實際持倉同步結果更新 STATES:
      - 部分成交 → 更新 s["qty"] 為交易所實際持倉數量
      - 完全未成交 → 清除 entry_count / last_entry_time / pending_side，
                       讓該幣種立即重回 ACTIVE 掃描模式
    """
    MAX_WAIT_SECONDS = 60.0  # 可調整：限價單最長容忍秒數
    ...
    while True:
        ...
        try:
            cancelled_list = await EXECUTION_ENGINE.cancel_stale_pending_orders(
                max_wait_seconds=MAX_WAIT_SECONDS,
                paper_trading=PAPER_TRADING
            )
        except Exception as e:
            print(f"❌ [超時撤單掃描器] cancel_stale_pending_orders 發生例外: {e}")
            continue

        for result in cancelled_list:
            sym      = result['symbol']
            filled   = result['filled_qty']
            side     = result['side'].lower()
            action   = result['action']  # 'cancelled' or 'already_done'

            # 將 ccxt 格式的 'BTC/USDT:USDT' 轉回內部 'BTCUSDT' 格式
            internal_sym = sym.replace('/', '').replace(':USDT', '').replace(':usdt', '')
            s = STATES.get(internal_sym) or STATES.get(sym)
            if not s:
                continue

            try:
                # 嘗試從交易所拿到最新實際持倉（不依賴 engine 回報的 filled_qty）
                ...
                if actual_pos:
                    # 部分成交：以交易所回報數量為準更新 qty
                    actual_qty = float(actual_pos.get('contracts', 0) or 0)
                    side_sign  = 1 if actual_pos.get('side', '') == 'long' else -1
                    s["qty"]   = actual_qty * side_sign
                    print(
                        f"📊 [超時撤單] {internal_sym} 撤單後實際持倉: {s['qty']:.4f} 張"
                        f" (原始預期: {result.get('original_quantity', '?')})"
                    )
                else:
                    # 完全未成交：重置追蹤狀態讓機器人重新進入掃描
                    print(
                        f"🔄 [狀態重置] {internal_sym} 限價單完全未成交 (filled=0)，"
                ...
                print(f"⚠️ [超時撤單] {internal_sym} 持倉同步失敗: {pe}")
