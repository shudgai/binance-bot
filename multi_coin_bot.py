...
def is_entry_volume_confirmed(sym, side):
    ...
    # 從幣種個性配置地圖讀取動態乘數，若無則使用預設值
    profile = COIN_PROFILES.get(sym, {})
    sl_multiplier = get_effective_exit_setting(sym, "sl_atr_multiplier", profile.get("sl_mult", SL_ATR_MULTIPLIER), side == "buy")
    tp_multiplier = get_effective_exit_setting(sym, "tp_atr_multiplier", profile.get("tp_mult", TP_ATR_MULTIPLIER), side == "buy")
