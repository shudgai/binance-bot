...
# ----------------- 'ATR' 模式參數 -----------------
SL_ATR_MULTIPLIER = 6.0   # 停損為 6 倍 ATR (原 4.0 → 放寬，避免正常波動假停損)
    ...
    async with order_lock:
        ...
        try:
            ...
            if PAPER_TRADING:
                ...
                # --- 掛載實體停利單 (保命單) ---
                # ✅ 修正：移除實體 STOP_MARKET 停損單，避免與軟體端 ATR 動態停損「雙層衝突」
                # 軟體端 monitor_position_tp_sl() 以 6x ATR 統一管理停損，更具彈性
                try:
                    ...
                    print(f"🛡️ [保命防護] 已成功掛載實體停利 (+0.2%): {tp_price}（停損由軟體 ATR 6x 動態管理）")
                except Exception as e:
                    print(f"⚠️ [保命防護] 掛載實體停利失敗: {e}")
...
async def watch_kline_and_strategy():
    ...
    # 防抖緩衝：regime 需連續確認 2 次才切換，避免邊界跳動造成無謂清倉
    if not hasattr(watch_kline_and_strategy, '_regime_confirm_count'):
        watch_kline_and_strategy._regime_confirm_count = {}
    ...
    while True:
        ...
        try:
            ...
            if macro_regime != last_market_condition:
                # ✅ 修正：防抖緩衝 — regime 需連續確認 2 次才觸發清倉，避免邊界跳動造成誤砍
                _confirm = watch_kline_and_strategy._regime_confirm_count
                _confirm[macro_regime] = _confirm.get(macro_regime, 0) + 1
                if _confirm[macro_regime] < 2:
                    print(f"⏳ [Regime 防抖] {last_market_condition}→{macro_regime} 第 {_confirm[macro_regime]}/2 次確認，等待下次確認...")
                else:
                    # 連續 2 次確認，正式切換
                    _confirm.clear()
                    print(f"🚨🚨 [大盤變天] 訊號由 {last_market_condition} 轉為 {macro_regime}（2 次確認通過）！")
                    fallback_price = closes_history[-1] if len(closes_history) > 0 else (ohlcv_buffer[-1][4] if ohlcv_buffer else 0)
                    current_fallback_p = current_p if 'current_p' in locals() else fallback_price
                    
                    # ✅ 修正：大盤變天只清「虧損倉位」，獲利倉保留讓 ATR 停損自然管理
                    if current_pos_qty > 0:
                        pos_profit_pct = (current_fallback_p - current_pos_avg) / current_pos_avg if current_pos_avg > 0 else 0
                        if pos_profit_pct < 0:
                            print(f"🧹 [大盤變天] 多單虧損 ({pos_profit_pct*100:.2f}%)，強制退場：{symbol}")
                            await close_entire_position('sell', abs(current_pos_qty), current_fallback_p, current_pos_avg, force_market=True)
                        else:
                            print(f"✅ [大盤變天] 多單獲利中 ({pos_profit_pct*100:.2f}%)，保留讓 ATR 停損管理，不強制砍倉")
                    elif current_pos_qty < 0:
                        pos_profit_pct = (current_pos_avg - current_fallback_p) / current_pos_avg if current_pos_avg > 0 else 0
                        if pos_profit_pct < 0:
                            print(f"🧹 [大盤變天] 空單虧損 ({pos_profit_pct*100:.2f}%)，強制退場：{symbol}")
                            await close_entire_position('buy', abs(current_pos_qty), current_fallback_p, current_pos_avg, force_market=True)
                        else:
                            print(f"✅ [大盤變天] 空單獲利中 ({pos_profit_pct*100:.2f}%)，保留讓 ATR 停損管理，不強制砍倉")
            ...
            if abs(current_pos_qty) > 0.000001:
                ...
                # 🚀 [智能防禦搶救機制] 提早動態停損
                # ✅ 修正 1：反轉搶救停損條件加嚴
                # 原條件：虧損 -1% + MACD 死叉（1分鐘K上極頻繁，造成大量假停損）
                # 新條件：虧損 -2% + MACD 死叉 + RSI < 45 + 收盤低於均價 1 個 ATR（多重確認）
                _atr_confirm = current_atr if current_atr > 0 else (current_pos_avg * 0.01)
                if not close_signal and is_long and profit_pct < -0.02 and current_rsi < 45 and \
                        (prev_macd_line > prev_macd_signal and macd_line < macd_signal) and \
                        close_price < current_pos_avg - _atr_confirm:
                    close_signal = True; close_reason = "📉 [反轉搶救] 多單趨勢轉弱(MACD死叉+RSI<45+ATR確認)，提早認賠！"
                if not close_signal and not is_long and profit_pct < -0.02 and current_rsi > 55 and \
                        (prev_macd_line < prev_macd_signal and macd_line > macd_signal) and \
                        close_price > current_pos_avg + _atr_confirm:
                    close_signal = True; close_reason = "📈 [反轉搶救] 空單趨勢轉強(MACD金叉+RSI>55+ATR確認)，提早認賠！"

                # ✅ 修正 2：時間停損延長，避免正常震盪被誤殺
                # 原設定：持倉 > 15 分鐘 (900s) + 虧損 > 1%
                # 新設定：持倉 > 30 分鐘 (1800s) + 虧損 > 2%（更嚴格的確認，給予倉位足夠呼吸空間）
                if not close_signal and (time.time() - position_open_time > 1800) and profit_pct < -0.02:
                    close_signal = True; close_reason = f"⏱️ [時間停損] 持倉已達 {(time.time()-position_open_time)/60:.1f} 分鐘且虧損 >{profit_pct*100:.1f}%，解除僵局！"
