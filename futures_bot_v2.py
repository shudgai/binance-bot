...
# 資金費率與滑價防護網參數
# ================================================================
FUNDING_RATE_SHORT_BLOCK = -0.005 # 如果資金費率小於此值(如-0.5%)，禁止做空以防高額利息
MAX_RETRY_COUNT = 3
SLIPPAGE_PENALTY_MULTIPLIER = 1.0
SLIPPAGE_RECOVERY_TIME = 0.0
...
async def execute_order_and_risk(side, price):
    ...
    async with order_lock:
        ...
        global SLIPPAGE_PENALTY_MULTIPLIER, SLIPPAGE_RECOVERY_TIME
        if time.time() > SLIPPAGE_RECOVERY_TIME and SLIPPAGE_PENALTY_MULTIPLIER < 1.0:
            print(f"♻️ [滑點懲罰解除] 懲罰時間已過，權重恢復為 1.0")
            SLIPPAGE_PENALTY_MULTIPLIER = 1.0
            
        actual_quote_amount = available_margin * 0.95 * SLIPPAGE_PENALTY_MULTIPLIER
        
        if actual_quote_amount < 6.0:
            print(f"🛑 [防護攔截] 剩餘可用額度 {actual_quote_amount:.2f} USDT 低於幣安最低名目價值限制 (5 USDT)，強制放棄避免產生孤兒倉位")
            return
            
        print(f"💰 [{macro_regime}] 下單 {actual_quote_amount:.2f} USDT (設定:{quote_amount:.0f}, 可用:{available_margin:.2f}, 權重:{SLIPPAGE_PENALTY_MULTIPLIER:.2f})")
            ...
            if PAPER_TRADING:
                ...
                if spread <= 0 or bid <= 0:
                    ...
                    while wait_time < 30:
                        ...
                        still_open = [oid for oid in valid_order_ids if oid in open_ids]
                        if not still_open:
                            print(f"✅ [限價等候] 所有網格單已完全成交！")
                            break
                            
                    # 超時處理：撤銷所有未成交訂單，並市價補齊
                    open_orders = await exchange.fetch_open_orders(symbol)
                    open_ids = [o['id'] for o in open_orders]
                    still_open = [oid for oid in valid_order_ids if oid in open_ids]
                    if still_open:
                        print(f"⚠️ [限價超時] 30秒已到，撤銷未成交的 {len(still_open)} 筆訂單，改市價單吃下")
                        unfilled_qty = 0.0
                        for oid in still_open:
                            try:
                                o = await exchange.fetch_order(oid, symbol)
                                unfilled_qty += o['remaining']
                                await exchange.cancel_order(oid, symbol)
                            except:
                                pass
                        
                        if unfilled_qty >= prec['min_qty']:
                            try:
                                print(f"⚡ [市價補齊] 補齊未成交數量: {unfilled_qty}")
                                market_order = await exchange.create_order(
                                    symbol=symbol, type='market', side=side, amount=unfilled_qty, params={'marginMode': 'isolated'}
                                )
                                # 更新 open_order 以便後續取平均價
                                open_order = market_order
                            except Exception as e:
                                print(f"🚨 [市價補齊失敗]: {e}")
                ...
                if slippage > 0.005:
                    print(f"⚠️ [滑點預警] 實際成交均價 {avg_price} 與預期 {price} 差距超過 0.5% ({slippage*100:.2f}%)")
                    SLIPPAGE_PENALTY_MULTIPLIER = 0.5
                    SLIPPAGE_RECOVERY_TIME = time.time() + 3600  # 1小時後恢復
                    print(f"📉 [風控降權] 未來 1 小時內的開倉資金將降至 50%")
                
                # [新增] 掛載實體停損與停利單 (保命單)
                try:
                    # 使用 ATR 計算動態停損距離 (3倍 ATR，但最大不超過 5%)
                    global current_atr
                    atr_sl_dist = current_atr * 3.0 if current_atr > 0 else avg_price * 0.03
                    max_sl_dist = avg_price * 0.05
                    actual_sl_dist = min(atr_sl_dist, max_sl_dist)
                    
                    tp_price = round_price(avg_price * 1.002 if side == 'buy' else avg_price * 0.998, prec['tick_size'])
                    sl_price = round_price(avg_price - actual_sl_dist if side == 'buy' else avg_price + actual_sl_dist, prec['tick_size'])
