...
def test_range_realtime_trailing_requires_persistent_confirmation():
    # Range 路線的移動停利改成跟 MA 路線一樣的連續多筆/秒數確認，不再等整根
    # K 棒收線；第一筆穿越只累積確認次數，還不足以出場。
    sym = "RANGEUSDT"
    init_states([sym])
    reset_coin_state(sym)
    state = STATES[sym]
    ...
    with patch("core.orders.close_position", AsyncMock()) as close:
        asyncio.run(update_trade_signal(
            sym, {"price": 100.29, "amount": 1.0, "timestamp": 1_000_000},
        ))

    close.assert_not_awaited()
    assert state["range_trailing_cross_count"] == 1


def test_range_realtime_trailing_exits_after_confirm_window():
    sym = "RANGEUSDT"
    init_states([sym])
    reset_coin_state(sym)
    state = STATES[sym]
    ...
    with patch("core.orders.close_position", AsyncMock()) as close:
        ...
        close.assert_not_awaited()
        asyncio.run(update_trade_signal(
            sym, {"price": 100.29, "amount": 1.0, "timestamp": 1_000_500},
        ))
        close.assert_not_awaited()
        asyncio.run(update_trade_signal(
            sym, {"price": 100.29, "amount": 1.0, "timestamp": 1_001_100},
        ))
